...
int main(int argc, char** argv)
{
    ...
    if(!private_n->getParam("odom_frame", odom_frame_id_)) {
        ROS_WARN("No odom_frame-str provided - default: odom");
        odom_frame_id_ = "odom";
    }
  ...
  ros::Rate loop_rate(d_exec_rate);
    ...
    while(ros::ok())
    {
        ...
        // if (swap_tickLR_flag == false) // default
          tmp_tickleft = (tick_vec.tick_lb + tick_vec.tick_lf) * 0.5 * rwheeltick_positive_factor * (-1);
          tmp_tickright = (tick_vec.tick_rb + tick_vec.tick_rf) * 0.5 * rwheeltick_positive_factor;
